...
{
    Task<PayoutPreviewResponseDto> GetPayoutPreviewAsync();
    Task<ExecutePayoutResponseDto> ExecutePayoutAsync(ExecutePayoutRequest request, Guid adminUserId);
}
...
public class PayoutService : IPayoutService
{
    ...
    public PayoutService(HomeBoardDbContext context)
    {
        _context = context;
    }

    public async Task<PayoutPreviewResponseDto> GetPayoutPreviewAsync()
    {
        // Get family settings for rate
        var settings = await _context.FamilySettings.FirstOrDefaultAsync();
        if (settings == null)
        {
            throw new InvalidOperationException("Family settings not found. Please configure point-to-money rate first.");
        }
    ...
    public async Task<ExecutePayoutResponseDto> ExecutePayoutAsync(ExecutePayoutRequest request, Guid adminUserId)
    {
        // Get family settings for rate
        var settings = await _context.FamilySettings.FirstOrDefaultAsync();
        if (settings == null)
